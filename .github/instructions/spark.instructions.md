---
applyTo: 'spark-sdd.agent.md'
---
## Memory Management

Maintain a structured memory system rooted at .specs/memory/
...
Read .specs/memory/memory.md at session start. Load specific topic files only when relevant.

Topic files:
- .specs/memory/general.md — cross-project conventions and preferences
...
MEMORY.md. The list lives in AGENTS.md (single source of truth). Project MEMORY.md has a
...
Read AGENTS.md for memory rules and topic files.

When a new file is added to .specs/memory/:
- Add it to the ## Global Memory topic file list in AGENTS.md only
...
(.specs/{projectName}/memory/). If it does not exist, create it:
...
1. Staging — knowledge accumulates in .specs/memory/domain/{name}/
