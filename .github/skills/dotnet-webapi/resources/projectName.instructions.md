---
applyTo: "{sourceRoot}/{namespaceName}/{projectName}/**, {sourceRoot}/{namespaceName}/{projectName}.Shared/**, {sourceRoot}/{namespaceName}/{projectName}.UnitTests/**, {sourceRoot}/{namespaceName}/{projectName}.IntegrationTests/**"
---
...
- `{sourceRoot}/{namespaceName}/{projectName}/{projectName}.UnitTests/` — xUnit unit test project [required]
- `{sourceRoot}/{namespaceName}/{projectName}/{projectName}.IntegrationTests/` — xUnit integration test project [required]
...
- Use xUnit for isolated unit tests that do not require the ASP.NET Core host.
- Mock dependencies using interfaces; do not spin up `WebApplicationFactory` here.

### {sourceRoot}/{namespaceName}/{projectName}.IntegrationTests Project Guidance

- The *.IntegrationTests project references the {projectName} service project.
- Use xUnit with `WebApplicationFactory<Program>` and `IClassFixture` for integration tests that exercise the full HTTP pipeline.
