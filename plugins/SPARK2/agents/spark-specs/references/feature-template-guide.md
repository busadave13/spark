...
- **Status** — lifecycle state of the feature spec. Valid values: `Draft`, `Approved`, `Implemented`
  ...
  - Only the resolved TDD agent may set `Implemented`, except when `feature-editor` determines during codebase exploration (Step 3) that the feature is already fully implemented — in that case `feature-editor` may set `Implemented` directly
