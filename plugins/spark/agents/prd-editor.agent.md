...
1. **Determine `{projectName}`** — extract the project name from the user's request or path (e.g., "Mockery"). If ambiguous, ask the user.
2. **Locate `.specs/` folder**: Search for `.specs/{projectName}/` starting from:
...
3. Set `{docs-root}` = the located `.specs/{projectName}` folder.
