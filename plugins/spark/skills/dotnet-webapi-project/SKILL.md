...
1. Gather inputs → 2. Compute path map and existing-state snapshot → 3. Reconcile namespace assets → 4. Scaffold service + write aligned source → 5. Optional shared project + reference → 6. Scaffold unit tests → 7. Direct verification → 8. Runtime validation → 9. Report.

## Execution Summary

Maintain one concise summary covering: `namespaceFolder`, `projectName`, `projectNameLowerCase`, `dotnetVersion`; the checklist selection (`includeShared`); every managed path (mark unselected optional projects as skipped); the resolved namespace workspace filename (and whether it was discovered or defaulted); the configured container base image and lowercase container repository; for each artifact whether it was created, updated, or already existed; the runtime validation outcome (including whether container publish used the local OCI runtime or the tarball fallback); warnings, skipped steps, fallback actions, and explicit blockers; and the exact files touched.
...
The item is **unchecked by default** — except: if Step 2 found the optional project already on disk (`sharedProjectExists`), pre-check it so the user must explicitly uncheck to skip an existing project.

Persist the choice as `includeShared` for the rest of the run. Do **not** proceed until confirmed.
...
# Discover existing namespace-level workspace (single-instance policy)
$workspaceMatches = @()
...
If exactly one workspace file is discovered, reuse it as-is — do **not** rename, copy, recreate, or overwrite.
...
1. **`src\{namespaceFolder}\.vscode\tasks.json`**
   - If missing: create `.vscode\` if needed, copy `resources\tasks.json`, replace `{projectName}` and `{namespaceFolder}` tokens.
   - If present: parse the JSON, leave unrelated tasks untouched, and ensure these skill-managed entries (matched by `label`) exist with the correct `args`/`options.cwd`/`presentation`:
     - `{projectName}: Build` and `{projectName}: Run` → service project
     - `{projectName}.UnitTests: Run` → unit test project
...
The templates in `resources\` are already aligned to the project AGENTS.md guidelines (Minimal API endpoint extension methods, HTTP-agnostic services, dedicated models, file-scoped namespaces), so they are written into place during scaffolding.
...
6. **Generated `.http` file** (`$httpFilePath`):
   ...
   - If the service project already existed and `$httpFilePath` is missing, surface this as an explicit blocker — do **not** synthesize one.
...
$openBrace  = [char]123
$closeBrace = [char]125
Select-String -Path $vscodeTasksPath -Pattern ([regex]::Escape('"{projectName}: Build"'))                                                  -Quiet
Select-String -Path $vscodeTasksPath -Pattern ([regex]::Escape('"{projectName}: Run"'))                                                    -Quiet
Select-String -Path $vscodeTasksPath -Pattern ([regex]::Escape('"{projectName}.UnitTests: Run"'))                                          -Quiet
Select-String -Path $vscodeTasksPath -Pattern ([regex]::Escape("{projectName}/{projectName}/{projectName}.csproj"))                        -Quiet
...
-not (Select-String -Path $vscodeTasksPath -Pattern ([regex]::Escape([string]$openBrace) + '(projectName|namespaceFolder)' + [regex]::Escape([string]$closeBrace)) -Quiet)
```
...
1. `dotnet test $unitTestsPath` — this exercises the full `/weatherforecast` request/response pipeline in-process via `WebApplicationFactory<Program>`, so it doubles as the HTTP smoke test.
2. Validate SDK container publishing for the service:
   ```powershell
...
3. **Cleanup**: delete any tarball files, temporary directories, or build outputs created solely for this validation. Do not leave validation-only artifacts anywhere.
...
| Mvc.Testing package          | — | [added (v{mvcTestingVersion}) / updated to v{mvcTestingVersion} / already existed] |
...
| Existing service is missing its `.http` file | Stop and surface the blocker — the file is the canonical record of available requests for this service. Resolve manually (e.g. restore from history) and rerun. |
