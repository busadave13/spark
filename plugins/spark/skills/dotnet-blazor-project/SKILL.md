...
description: "Bootstrap or reconcile the repo-specific .github/instructions/{project}.instructions.md file for an existing Spark-managed .NET Blazor codebase. Use only when the user explicitly asks to create or update that instruction file, or when Spark/TDD has already identified missing repo instructions as a dependency. Do not use for end-user requests like 'create a new project', PRD creation, architecture creation, or app scaffolding."
---
